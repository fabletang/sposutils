...
import apache.http.annotation.ThreadSafe;
 ...
 * {@link #lease(Object, Object, apache.http.concurrent.FutureCallback)} method in order for the lease operation
 * to complete.
 *
 ...
 */
@ThreadSafe
public abstract class AbstractConnPool<T, C, E extends apache.http.pool.PoolEntry<T, C>>
                                               implements ConnPool<T, E>, ConnPoolControl<T> {

    private final Lock lock;
    private final ConnFactory<T, C> connFactory;
    private final Map<T, apache.http.pool.RouteSpecificPool<T, C, E>> routeToPool;
    ...
    public AbstractConnPool(
        ...
        this.routeToPool = new HashMap<T, apache.http.pool.RouteSpecificPool<T, C, E>>();
    ...
    public void shutdown() throws IOException {
        ...
        try {
            ...
            for (final apache.http.pool.RouteSpecificPool<T, C, E> pool: this.routeToPool.values()) {
                pool.shutdown();
            }
    ...
    private apache.http.pool.RouteSpecificPool<T, C, E> getPool(final T route) {
        apache.http.pool.RouteSpecificPool<T, C, E> pool = this.routeToPool.get(route);
        if (pool == null) {
            pool = new apache.http.pool.RouteSpecificPool<T, C, E>(route) {
     ...
     */
    public Future<E> lease(final T route, final Object state, final apache.http.concurrent.FutureCallback<E> callback) {
    ...
    private E getPoolEntryBlocking(
        ...
        this.lock.lock();
        try {
            final apache.http.pool.RouteSpecificPool<T, C, E> pool = getPool(route);
            E entry = null;
            while (entry == null) {
                Asserts.check(!this.isShutDown, "Connection pool shut down");
                for (;;) {
                    ...
                    if (freeCapacity > 0) {
                        ...
                        if (totalAvailable > freeCapacity - 1) {
                            if (!this.available.isEmpty()) {
                                ...
                                lastUsed.close();
                                final apache.http.pool.RouteSpecificPool<T, C, E> otherpool = getPool(lastUsed.getRoute());
    ...
    public void release(final E entry, final boolean reusable) {
        ...
        try {
            if (this.leased.remove(entry)) {
                final apache.http.pool.RouteSpecificPool<T, C, E> pool = getPool(entry.getRoute());
    ...
    public apache.http.pool.PoolStats getTotalStats() {
        this.lock.lock();
        try {
            return new apache.http.pool.PoolStats(
                    this.leased.size(),
                    this.pending.size(),
                    this.available.size(),
                    this.maxTotal);
    ...
    public apache.http.pool.PoolStats getStats(final T route) {
        Args.notNull(route, "Route");
        this.lock.lock();
        try {
            final apache.http.pool.RouteSpecificPool<T, C, E> pool = getPool(route);
     ...
     * @since 4.3
     */
    protected void enumAvailable(final apache.http.pool.PoolEntryCallback<T, C> callback) {
        this.lock.lock();
        try {
     ...
     * @since 4.3
     */
    protected void enumLeased(final apache.http.pool.PoolEntryCallback<T, C> callback) {
        this.lock.lock();
        try {
    ...
    private void purgePoolMap() {
        final Iterator<Map.Entry<T, apache.http.pool.RouteSpecificPool<T, C, E>>> it = this.routeToPool.entrySet().iterator();
        while (it.hasNext()) {
            final Map.Entry<T, apache.http.pool.RouteSpecificPool<T, C, E>> entry = it.next();
            final apache.http.pool.RouteSpecificPool<T, C, E> pool = entry.getValue();
    ...
    public void closeIdle(final long idletime, final TimeUnit tunit) {
        ...
        enumAvailable(new apache.http.pool.PoolEntryCallback<T, C>() {

            public void process(final apache.http.pool.PoolEntry<T, C> entry) {
