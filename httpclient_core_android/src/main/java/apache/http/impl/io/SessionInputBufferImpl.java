...
import apache.http.util.ByteArrayBuffer;
import apache.http.util.CharArrayBuffer;
import apache.http.MessageConstraintException;
 ...
 * <p/>
 * {@link #readLine(apache.http.util.CharArrayBuffer)} and {@link #readLine()} methods of this
 ...
 */
@NotThreadSafe
public class SessionInputBufferImpl implements SessionInputBuffer, apache.http.io.BufferInfo {

    private final HttpTransportMetricsImpl metrics;
    private final byte[] buffer;
    private final ByteArrayBuffer linebuffer;
    private final int minChunkLimit;
    private final apache.http.config.MessageConstraints constraints;
    ...
    public SessionInputBufferImpl(
        ...
        apache.http.util.Args.notNull(metrics, "HTTP transport metrcis");
        apache.http.util.Args.positive(buffersize, "Buffer size");
        this.metrics = metrics;
        this.buffer = new byte[buffersize];
        this.bufferpos = 0;
        this.bufferlen = 0;
        this.minChunkLimit = minChunkLimit >= 0 ? minChunkLimit : 512;
        this.constraints = constraints != null ? constraints : apache.http.config.MessageConstraints.DEFAULT;
    ...
    private int streamRead(final byte[] b, final int off, final int len) throws IOException {
        apache.http.util.Asserts.notNull(this.instream, "Input stream");
    ...
    public int readLine(final CharArrayBuffer charbuffer) throws IOException {
        apache.http.util.Args.notNull(charbuffer, "Char array buffer");
        int noRead = 0;
        boolean retry = true;
        while (retry) {
